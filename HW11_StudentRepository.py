...
class Student:
    ...
    def __init__(self, person_info, major_info) -> None:
...
class Major:
    ...
    def update_major(self, flag, course) -> None:
    ...
    def left_required(self, courses) -> List:
    ...
    def left_electives(self, courses) -> List:
...
class University:
    ...
    def __init__(self, dir_path) -> None:
        ...
        self.student_grades_table_db(
            os.path.join(dir_path, "810_startup.db"))

    def students_DB(self) -> None:
    ...
    def student_grades_table_db(self, db_path) -> List[Tuple]:
        """ generate the prettytable for student grade summary """
        pt = PrettyTable(
        ...
        query: str = """ select s.Name, s.CWID, g.Course, g.Grade, i.Name
                    from  "students.txt" as s
                    join "grades.txt" as g on s.CWID = g.StudentCWID
                    join "instructors.txt" as i on i.CWID = g.InstructorCWID
                    order by s.Name """
...
def main() -> None:
    cwd: str = "G:/Stevens/Courses/Sem4/Assignment/SSW810_Python/StudentRepository_SSW810/input_11/"
    University(cwd)
